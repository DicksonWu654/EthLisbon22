import { Button, Container, HStack, Input, NumberInput, NumberInputField, Text, VStack } from '@chakra-ui/react'
import { Web3Button,
  useAccount, 
  useBalance, 
  useToken, 
  ...
  return (
    <Container centerContent minH={'80vh'} justifyContent={'center'}>
      { account.isConnected == true && (
        <VStack maxW={'xl'} gap={5}>
          <NumberInput value={timeDelay}>
            <NumberInputField
          ...
          <Input placeholder={'Panic Address'} onChange={e => {setPanicAddress(e.target.value)}} />
          <Button colorScheme={'blue'} w={'100%'} onClick={createWallet}>Create Wallet Contract</Button>
        </VStack>
      )}
    </Container>
  )
